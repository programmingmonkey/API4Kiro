...
- **Antigravity（Google）登录的 client secret 不在源码，也不进公开 Release / CI vsix。** 默认 `npm run package` 不读本地 `antigravity.secret`；只有显式设置 `A2K_EMBED_ANTIGRAVITY_SECRET=1` 或环境变量 `A2K_ANTIGRAVITY_CLIENT_SECRET` 才会注入，且这样打出的包只供本机自用，禁止 `gh release upload`、禁止推进公开仓。从源码构建者若需要 Antigravity 登录，须自备 Google OAuth client 并在本机构建期注入（`esbuild.js`）。v4.13.57 曾有一份本机含密钥包被上传到 Release，该资产已原地替换为无密钥包；建议在 Google Cloud Console 轮换该 OAuth client secret。
